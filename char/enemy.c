...
#ifdef _ENEMYTEMP_OPTIMUM // Robin È¡³ö×î´óENEMYTEMP ID
...
#ifdef _ENEMY_OPTIMUM // Robin È¡³ö×î´óENEMY ID
...
#define	GROUP_STARTINTNUM		2
...

		{
			...
			if( enemycnt == 0 ) {
                fprint("error :ÍÅ¶ÓÉè¶¨ÖÐµÐÈËÉÐÎ´Éè¶¨ file:%s line:%d\n",filename,linenum);
				continue;;
			}
            if( checkRedundancy( &GROUP_group[group_readlen].intdata[ENEMY_ID1],
            			CREATEPROB1 - ENEMY_ID1))
            {
                fprint("error :ÍÅ¶ÓÉè¶¨ÖÐµÐÈËÖØ¸´Éè¶¨ file:%s line:%d\n",filename,linenum);
				continue;;
			}
...
#endif
#ifdef _SKILL_NOCAST
    PETSKILL_NOCAST,              //(547)³ÁÄ¬      vincent add 2002/07/16
#endif
#ifdef _SKILL_ROAR
...
BOOL PETFUSION_getIndexForChar( int toindex, int *MainIndex, int *Subindex1, int *Subindex2, char *data)
{
	...
	for( i=0; i<3; i++)	{
		...
		if( CHAR_getInt( petindex, CHAR_TRANSMIGRATION ) > 1 ) {
			CHAR_talkToCli( toindex, -1, "¶þ×ª³èÎï²»ÄÜÈÚºÏ¡£", CHAR_COLORYELLOW);
			return FALSE;
		}
#endif
		if( CHAR_getInt( toindex, CHAR_RIDEPET ) == pindex[i]){
			CHAR_talkToCli( toindex, -1, "Æï³ËÖÐµÄ³èÎï²»ÄÜÈÚºÏ", CHAR_COLORYELLOW);
			return FALSE;
		}
		if( i == 0 ) {
			*MainIndex = petindex;
...
int EVOLUTION_createPetFromEnemyIndex( int charaindex, int baseindex, int flg)
{
	...
	int		base[4]={0,0,0,0};
	int		illegalpetskill[15] = {41,52,600,601,602,603,604,614,617,628,630,631,635,638,641};//²»¿ÉÒÅ´«µÄ³è¼¼
#define		E_PAR( a)		(*(p + (a)))
#define		ET_PAR( a)		(*(tp + (a)))
#define		PARAM_CAL( l) 	(( level -1)*ET_PAR( E_T_LVUPPOINT)+ ET_PAR(E_T_INITNUM)) * ET_PAR( (l))
	havepetelement=-1;
...
//------------------------------------------------------
	{
		...
		for( i=0; i < CHAR_MAXPETSKILLHAVE; i ++ ) {//³è¼¼
			int j;
			for( j=0; j<15; j++)	{	//¼ì²é·Ç·¨¼¼ÄÜ
	...
	if( flg == 1 ){
		havepetelement = CHAR_getCharPetElement( charaindex);//ÕÒ³ö³èÎï¿ÕÎ»
		if( havepetelement < 0 ) return -1;
	}
//------------------------------------------------------
...
int PET_CheckIncubate( int charaindex)
{
	...
	for(i=0 ; i < CHAR_MAXPETHAVE ; i++)	{
		...
		if( CHAR_getInt( petindex, CHAR_FUSIONBEIT) != 1 ||
			CHAR_getInt( petindex, CHAR_FUSIONRAISE) <= 0 )return;//¼ì²éÊÇ·ñÎªÈÚºÏ³è
		...
		if( (int)checkeage > (time_l+(anhour*2)) ){
			...
			sprintf( buf, "µ°¡´%s¡µµÄÆ·ÖÊ±ä²îÁË¡£", CHAR_getUseName( petindex ));
			...
			char buf[256];
			sprintf( buf, "µ°¡´%s¡µ³ÊÏÖ¿É  Ê³×´Ì¬¡£", CHAR_getUseName( petindex ));
			CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
		}
	}
	return 1;
		...
		if( CHAR_getInt( charaindex, CHAR_FUSIONBEIT) != 1 ||
			CHAR_getInt( charaindex, CHAR_FUSIONRAISE) <= 0 ) return 0;//¼ì²éÊÇ·ñÎªÈÚºÏ³è
...
#ifdef _PET_FUSIONSHOWTIME
		{
			...
			for(i=0 ; i < CHAR_MAXPETHAVE ; i++)	{
				...
				if( deftime >= 0 ){
					sprintf( caname, "³èÎïµ°(%d)", (int)(deftime/60));
				}else{
					deftime =  ( checkeage - oldtime) - anhour;
					sprintf( caname, "³èÎïµ°(¶ö%d)", (int)(deftime/60));
				}
		...
		if( (int)checkeage > (time_l+(anhour*2.5)) ){
			...
			sprintf( buf, "µ°¡´%s¡µµÄÆ·ÖÊ±ä²îÁË¡£", CHAR_getUseName( charaindex ));
			...
			char buf[256];
			sprintf( buf, "µ°¡´%s¡µ³ÊÏÖ¿É  Ê³×´Ì¬¡£", CHAR_getUseName( charaindex ));
			CHAR_talkToCli( masterindex, -1, buf, CHAR_COLORYELLOW);
		}
		return 1;
#endif
